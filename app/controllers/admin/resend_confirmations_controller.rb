  ...
  class ResendConfirmationsController < AdminController
    ...
    def create
      ...
      unconfirmed_rsvps.each do |rsvp|
        updated_attributes = []
        updated_attributes << "date" if rsvp.meeting.happens_at_updated_at > rsvp.confirmation_sent_at
        updated_attributes << "location" if rsvp.meeting.location_updated_at > rsvp.confirmation_sent_at
